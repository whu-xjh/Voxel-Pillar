...
def load_txt_trajectory(txt_path, t_min=None, t_max=None, t_B_prism=None, skip_header=True):
    """
    Load trajectory from TXT/CSV file.
    ...
    try:
        with open(txt_path, 'r') as f:
            ...
            has_header = any(not c.replace(',', '').replace('.', '').replace('-', '').replace('+', '').replace('e', '').replace(' ', '').isdigit()
                           for c in first_line if c.isalpha() or c in '_,')
            # Detect delimiter: comma if first_line contains comma and has spaces, else space
    ...
    # Handle different column orderings
    # Expected: timestamp, x, y, z, qx, qy, qz, qw
    # Or: timestamp, qx, qy, qz, qw, x, y, z (TUM format)
    if data.shape[1] == 8:
        t = data[:, 0]
        P = data[:, 1:4]
        Q = data[:, 4:8]
        # Check if quaternion format is xyzw or wxyz
        # Usually txt files have xyzw, convert to wxyz
...
def plot_trajectory_alignment(traj_gt, traj_est, save_path=None):
    """Plot trajectory alignment in multiple views."""
...
def evaluate_txt_trajectory(
    ...
    *,
    ...
    print(f"Loading estimated trajectory from: {txt_path}")
    ...
    # Ensure both trajectories have the same number of poses after interpolation
    if traj_gt_interp.num_poses != traj_est.num_poses:
        # Filter traj_est to match the interpolated GT timestamps
        valid_indices = []
        gt_timestamps = traj_gt_interp.timestamps
        for i, ts in enumerate(traj_est.timestamps):
            if ts in gt_timestamps:
                valid_indices.append(i)
        # Update traj_est to use only matching timestamps
    ...
    print(f"Aligning using first {n_align}/{n_total} poses ({align_fraction*100}%)")
    ...
    # Re-interpolate GT after alignment to ensure matching timestamps
    traj_gt_interp = trajectory_interpolation(traj_gt, traj_est.timestamps)
...
def main():
    ...
    parser.add_argument("--pred", required=True, help="Path to predicted trajectory file")
    ...
    parser.add_argument("--t-b-prism", nargs=3, type=float, default=[0, 0, 0],
                       help="Sensor to body transformation (x y z)")
    parser.add_argument("--align-fraction", type=float, default=1.0,
                       help="Fraction of trajectory to use for alignment (0-1]")
    ...
    result["sequence"] = os.path.splitext(os.path.basename(args.pred))[0]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        if os.path.exists(pred_path) and os.path.exists(gt_path):
            ...
            print("Default file paths not found. Please use command-line arguments:")
            print("python evaluate_trajectory.py --pred <pred_file> --gt <gt_file>")
    else:
        main()
